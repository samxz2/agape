...
<template>
  ...
  <!-- Drawer -->
  <div
    class="fixed top-0 right-0 h-full w-full max-w-sm z-50 bg-white border-l border-brown-200 shadow-2xl flex flex-col transition-all duration-[400ms] ease-out-expo"
    :class="cart.isOpen ? 'translate-x-0' : 'translate-x-full'"
  >
    <!-- Header -->
    <div class="border-b border-brown-200 px-5 py-4 bg-brown-600">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="relative">
            <ShoppingBag :size="24" class="text-cream-200" />
            <span
              v-if="cart.totalItems > 0"
              class="absolute -top-2 -right-2 bg-gold-400 text-brown-800 text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 shadow-lg"
            >
              {{ cart.totalItems }}
            </span>
          </div>
          <div>
            <h2 class="font-playfair text-lg text-cream-50 font-bold">Tu Carrito</h2>
        ...
        <div class="flex items-center gap-2">
          <span class="text-[10px] text-cream-300 bg-brown-500/50 px-2 py-0.5 rounded-full">
            {{ currencyStore.currency === 'USD' ? '$' : 'Bs.' }}
          </span>
          <button
            @click="cart.toggleCart()"
            class="text-cream-300 hover:text-cream-50 p-2 rounded-lg hover:bg-brown-500/50 transition-all"
          >
            <X :size="20" />
          </button>
        </div>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-3 scrollbar-thin">
      ...
      >
        ...
        <button
          @click="cart.toggleCart()"
          class="bg-brown-600 text-cream-50 px-8 py-3 rounded-2xl text-sm font-bold hover:bg-brown-500 transition-all shadow-lg"
        >
          Ver productos
        </button>
      </div>
      ...
      <div
        ...
        :key="item.id"
        class="bg-cream-50 rounded-2xl p-3 border border-brown-100 flex gap-3 items-center"
      >
        <img
          :src="item.imagen"
          :alt="item.nombre"
    ...
    <div
      v-if="cart.items.length > 0"
      class="border-t border-brown-200 px-5 py-4 bg-white space-y-3"
    >
