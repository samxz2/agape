...
export const useCartStore = defineStore('cart', () => {
  ...
  function agregarAlCarrito(producto: Producto, cantidad?: number) {
    ...
    if (existente) {
      existente.cantidad = cantidad ?? existente.cantidad + 1
    } else {
      items.value.push({ ...producto, cantidad: cantidad ?? 1 })
    }
    isOpen.value = true
  ...
  function generarMensajeWhatsapp() {
    ...
    const moneda = currencyStore.currency === 'USD' ? 'USD' : 'Bs.'
    ...
    items.value.forEach(item => {
      ...
      const precioMoneda = currencyStore.currency === 'USD' 
        ? `$${precioUSD.toFixed(2)}` 
        : `Bs. ${(precioUSD * tasaCambio.value).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, '.')}`
      
      const subtotalMoneda = currencyStore.currency === 'USD'
    ...
    const totalMoneda = currencyStore.currency === 'USD'
